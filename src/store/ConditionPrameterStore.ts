...
export class ConditionParameterStore {
    getAll(conditionObjectId: string): ConditionParameterDTO[] {
        return [1, 2, 3, 4].map(i => {
            return {
                id: `${conditionObjectId}--${i}`,
                name: `Table #${conditionObjectId}--${i}`,
                description: `Decription #${conditionObjectId}--${i}`
                // adm_Object: {
                //     id: `${i * 10}`,
                //     name: `Adm name #${i * 10}`,
